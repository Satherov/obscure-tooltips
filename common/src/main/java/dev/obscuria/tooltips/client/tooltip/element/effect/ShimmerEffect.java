...
public record ShimmerEffect(
    ...
    public static final MapCodec<ShimmerEffect> CODEC;

    @Override
    public MapCodec<ShimmerEffect> codec() {
        return CODEC;
    }
    ...
    private void renderSide(int segments,
        ...
        for (int i = 0; i < segments; i++) {
            ...
            if (flip) {
                GraphicUtils.color(buffer.addVertex(matrix, inner1.x, inner1.y, 0f), innerColor.lerp(accentColor, t1));
                GraphicUtils.color(buffer.addVertex(matrix, inner2.x, inner2.y, 0f), innerColor.lerp(accentColor, t2));
                GraphicUtils.color(buffer.addVertex(matrix, outer2.x, outer2.y, 0f), outerColor);
                GraphicUtils.color(buffer.addVertex(matrix, outer1.x, outer1.y, 0f), outerColor);
            } else {
                GraphicUtils.color(buffer.addVertex(matrix, inner2.x, inner2.y, 0f), innerColor.lerp(accentColor, t2));
                GraphicUtils.color(buffer.addVertex(matrix, inner1.x, inner1.y, 0f), innerColor.lerp(accentColor, t1));
                GraphicUtils.color(buffer.addVertex(matrix, outer1.x, outer1.y, 0f), outerColor);
